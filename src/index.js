// falllegal SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from falllegal/index.html · 138718 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "falllegal" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// FallLegal v1.0.0 · prime 743 · MIT
// Sovereign matter management + UK law engine for SRA-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='falllegal';const VERSION='1.0.0';const PRIME=743;const STORE='falllegal';const CONFIG_V='falllegal@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','matters','advice','corpus','weaves','audit','settings'];
...
let state={
 active:'dashboard',matterTab:'overview',activeMatterId:null,
 firm:null,advisers:[],clients:[],matters:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.matters=(await idbAll('matters'))||[];
 ...
 if(meta){state.activeMatterId=meta.activeMatterId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.matterTab=meta.matterTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeMatterId:state.activeMatterId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,matterTab:state.matterTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const matterId=(payload&&payload.matterId)||state.activeMatterId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+matterId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,matterId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveMatter(m,reason,noBc){m.updatedAt=Date.now();await idbPut('matters',m);if(!state.matters.find(x=>x.id===m.id))state.matters.push(m);else state.matters=state.matters.map(x=>x.id===m.id?m:x);await audit('matter.updated',reason||'Matter updated',{id:m.id,matterId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('matter.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Legal opinion signed and retained per SRA 13.5',{id:a.id,matterId:a.matterId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,matters:state.matters});
 ...
 for(const mt of(p.matters||[])){const local=state.matters.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveMatter(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='matter.created'||m.type==='matter.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.matters.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveMatter(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',matterId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
...
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForMatter:''}},
...
function newMatterRec(clientId){
 const seq=(state.firm&&state.firm.nextMatterRefSeq)||(state.matters.length+1);
 if(state.firm)state.firm.nextMatterRefSeq=seq+1;
...
function activeMatter(){return state.matters.find(m=>m.id===state.activeMatterId)}
function matterClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 mt.demo=true;
 await saveMatter(mt,'demo seed',true);
 state.activeMatterId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setMatterTab(id){state.matterTab=id;saveMeta();render()}
function selectMatter(id){state.activeMatterId=id;state.active='matter';state.matterTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activeMatter();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no matter selected';$('#mpMeta').textContent='choose from sidebar'}
...
function renderSidebar(){
 ...
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(matterClient(m)?clientLabel(matterClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 ...
 sb.innerHTML=`
 <h4>Matters · ${state.matters.length}</h4>
 ...
 <button class="btn primary sm" onclick="newMatter()">+ new matter</button>
 </div>
 <div class="matter-list">${matters.length?matters.map(m=>{
 const cl=matterClient(m);
 ...
 return `<div class="matter-card ${state.activeMatterId===m.id?'active':''}" onclick="selectMatter('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no matters yet · click + new matter</div>'}</div>`;
}
function render(){
 ...
 else if(a==='matters')v.innerHTML=viewMatters();
 else if(a==='matter')v.innerHTML=viewMatter();
...
function disclaimer(){return `<div class="disclaimer"><strong>FallLegal</strong> is a tool for SRA-regulated UK solicitors. It assists with matter management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or legal opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to FallLegal</h1>
 <p class="lead">Sovereign matter management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>paralegal</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.matters.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.matters.filter(m=>m.status==='closed').length;
 const critical=state.matters.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.matters){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">SRA ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 <div class="card"><h3>Matters</h3>
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.matters.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no matters</div>'}
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectMatter('${m.id}')">open →</button></div>`
...
function viewMatters(){
 const mm=state.matters.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Matters · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newMatter()">+ new matter</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=matterClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectMatter('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleSolicitorId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no matters · click + new matter</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewMatter(){
 ...
 if(!m)return disclaimer()+'<div class="no-active"><h3>No matter selected</h3><p>Choose one from the sidebar, or <a onclick="newMatter()">create new</a>.</p></div>';
 const cl=matterClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeMatter('${m.id}')">${m.status==='closed'?'reopen':'close matter'}</button>
 <button class="btn sm ox" onclick="deleteMatter('${m.id}')">delete</button>
 </div>
 </div>
 <div class="matter-tabs">${tabs.map(t=>`<button class="${state.matterTab===t[0]?'active':''}" onclick="setMatterTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.matterTab==='overview'?tabOverview(m,cl):''}
 ${state.matterTab==='retainer'?tabRetainer(m):''}
 ${state.matterTab==='conflict'?tabConflict(m,cl):''}
 ${state.matterTab==='timeline'?tabTimeline(m):''}
 ${state.matterTab==='advice'?tabAdvice(m):''}
 ${state.matterTab==='documents'?tabDocs(m):''}
 ${state.matterTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Matter detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updMatter('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updMatter('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updMatter('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updMatter('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible solicitor</label><select onchange="updMatter('responsibleSolicitorId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleSolicitorId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updMatter('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updMatter('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updMatter('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updMatter('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updMatter('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updMatter('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updMatter('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','legal-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updMatter('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updMatter('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updMatter('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updMatter('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updMatter('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updMatter('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updMatter('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updMatter('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local matters + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (matter)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForMatter)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this matter</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updMatter('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updMatter('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updMatter('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.matterId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue legal opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>falllegalpaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>falllegalpractice</strong>. This view shows ledger ref IDs.</p>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic legal patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeMatterId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeMatterId}','${w.id}')">apply to active matter</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeMatterId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeMatter().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / matter.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Matter</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.matters.find(x=>x.id===a.matterId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updMatter(field,val){const m=activeMatter();if(!m)return;m[field]=val;saveMatter(m,'field '+field+' updated')}
async function newMatter(){
 const m=newMatterRec();state.activeMatterId=m.id;
 await saveMatter(m,'new matter');await audit('matter.created','New matter created',{id:m.id,matterId:m.id,ref:m.ref});broadcast('matter.created',m);
 state.active='matter';state.matterTab='overview';saveMeta();render();
}
async function closeMatter(id){const m=state.matters.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveMatter(m,'matter '+m.status);if(m.status==='closed')broadcast('matter.closed',m);else broadcast('matter.reopened',m);render()}
async function deleteMatter(id){if(!confirm('Delete this matter? (audit log preserved)'))return;await idbDel('matters',id);state.matters=state.matters.filter(m=>m.id!==id);if(state.activeMatterId===id)state.activeMatterId=null;await audit('matter.deleted','Matter deleted',{id});render()}
async function sendClientCare(id){const m=state.matters.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveMatter(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.matters.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveMatter(m,'event added');render()}
async function addFileRef(id){const m=state.matters.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveMatter(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.matters.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveMatter(m,'file ref removed');render()}
async function issueAdvice(matterId){
 const m=state.matters.find(x=>x.id===matterId);if(!m)return;
...
async function runConflictCheck(matterId){
 ...
 await saveMatter(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— FallLegal T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeMatter();const cl=m?matterClient(m):null;
 const ctx=m?`\nActive matter: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK legal research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated legal advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— FallLegal T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('falllegal-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,matters:state.matters,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('falllegal-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.matters.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('matters',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.matters=state.matters.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeMatterId&&!state.matters.find(m=>m.id===state.activeMatterId))state.activeMatterId=null;
 await audit('demo.purged','Demo data removed',{matters:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','FallLegal booted',{version:VERSION,prime:PRIME});
})();
